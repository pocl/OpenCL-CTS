                                  ...
                                  TExplicitMemoryOrderType order_failure)
{
    ...
    if constexpr (is_host_atomic_fp_v<AtomicType>)
    {
        ...
        // this is necessary so that (*a == *exp) evaluates to true when comparing NANs
        if (std::memcmp((const void*)a, expected, sizeof(CorrespondingType)) == 0)
        {
    ...
    else
    {
        ...
        tmp = 0;
#endif
        if (std::memcmp((const void*)&tmp, expected, sizeof(CorrespondingType)) == 0)
          return true;
        *expected = tmp;
    }
    return false;
